...
**모든 작업 전 `엔진_로직_명세.md` 를 먼저 확인한다.** 위 `@` 로 자동 로드됨.

- 확정된 비즈니스 로직(박스소분 89개, 매입가 우선순위, basePP_v3, 추천 분기 등)은
  명세서에 있다 — **사용자에게 다시 묻지 않는다.**
- 불명확하면 추측하지 말고: ① 엔진_로직_명세.md ② 코드 ③ SQL(execute_sql) 순으로 확인.
- 화면(브라우저)은 캐시로 stale 일 수 있다 — 분석·검증은 API 응답 / SQL 로 한다.
- 코드 변경 후 반드시 API/SQL 로 결과를 검증하고 보고한다. 검증 없는 "완료" 금지.
- 로직을 변경하면 `엔진_로직_명세.md` 도 같이 갱신한다 (문서-코드 동기화).
